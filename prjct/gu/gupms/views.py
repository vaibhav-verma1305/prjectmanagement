...
def signup(request):
    global name, email, password, confirm_password
    if request.method == "POST":
        m=mysql.connect(host="localhost",user="root",passwd="7754092636",database='students')
        cursor=m.cursor()
        d=request.POST
        ...
        c = "insert into signupdetails values('{}','{}','{}','{}')".format(name,email,password,confirm_password)
        cursor.execute(c)
        m.commit()
...
def signin(request):
    ...
    if request.method =="POST":
        ...
        for key,value in d.items():
            ...
            if key=="password":
                password=value
        c="select * from signindetails where username='{}' and password='{}'".format(username,password)
        cursor.execute(c)
